...
use DB;
class _Pharmacyhospital_history extends Model
{ 
    public static function pharmacyhospital_history(){
        // syncronize pharmacyhospital_history table from offline to online
        $pharmacy_offline = DB::table('pharmacyhospital_history')->get();  
        foreach($pharmacy_offline as $pharmacy_offline){  
            $pharmacy_offline_count = DB::connection('mysql2')->table('pharmacyhospital_history')->where('phh_id', $pharmacy_offline->phh_id)->get();
                if(count($pharmacy_offline_count) > 0){ 
                    if($pharmacy_offline->updated_at > $pharmacy_offline_count[0]->updated_at){  
                        DB::connection('mysql2')->table('pharmacyhospital_history')->where('phh_id', $pharmacy_offline->phh_id)->update([    
                            'phh_id'=> $pharmacy_offline->phh_id,
                        ...
                        DB::table('pharmacyhospital_history')->where('phh_id', $pharmacy_offline_count[0]->phh_id)->update([  
                            'phh_id'=> $pharmacy_offline_count[0]->phh_id,
        ...
        $pharmacy_online = DB::connection('mysql2')->table('pharmacyhospital_history')->get();
        foreach($pharmacy_online as $pharmacy_online){  
            $pharmacy_online_count = DB::table('pharmacyhospital_history')->where('phh_id', $pharmacy_online->phh_id)->get();
                if(count($pharmacy_online_count) > 0){
                    DB::table('pharmacyhospital_history')->where('phh_id', $pharmacy_online->phh_id)->update([
                        'phh_id'=> $pharmacy_online->phh_id,
