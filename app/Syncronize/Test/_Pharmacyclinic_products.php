...
use DB;
class _Pharmacyclinic_products extends Model
{ 
    public static function pharmacyclinic_products(){
        // syncronize pharmacyclinic_products table from offline to online
        $pharmacy_offline = DB::table('pharmacyclinic_products')->get();  
        foreach($pharmacy_offline as $pharmacy_offline){  
            $pharmacy_offline_count = DB::connection('mysql2')->table('pharmacyclinic_products')->where('product_id', $pharmacy_offline->product_id)->get();
                if(count($pharmacy_offline_count) > 0){ 
                    if($pharmacy_offline->updated_at > $pharmacy_offline_count[0]->updated_at){  
                        DB::connection('mysql2')->table('pharmacyclinic_products')->where('product_id', $pharmacy_offline->product_id)->update([    
                            ...
                            'unit_price'=>$pharmacy_offline->unit_price,
                        ...
                        DB::table('pharmacyclinic_products')->where('product_id', $pharmacy_offline_count[0]->product_id)->update([  
                            ...
                            'unit_price'=>$pharmacy_offline_count[0]->unit_price,
        ...
        $pharmacy_online = DB::connection('mysql2')->table('pharmacyclinic_products')->get();
        foreach($pharmacy_online as $pharmacy_online){  
            $pharmacy_online_count = DB::table('pharmacyclinic_products')->where('product_id', $pharmacy_online->product_id)->get();
                if(count($pharmacy_online_count) > 0){
                    DB::table('pharmacyclinic_products')->where('product_id', $pharmacy_online->product_id)->update([
                        ...
                        'unit_price'=>$pharmacy_online->unit_price,
