...
use DB;
class _Gtc_cart_patient_confirm_cancel extends Model
{ 
    public static function gtc_cart_patient_confirm_cancel(){ 
        // syncronize gtc_cart_patient_confirm_cancel table from offline to online   
        $gtc_offline = DB::table('gtc_cart_patient_confirm_cancel')->get();  
        foreach($gtc_offline as $gtc_offline){  
            $gtc_offline_count = DB::connection('mysql2')->table('gtc_cart_patient_confirm_cancel')->where('cart_cncl', $gtc_offline->cart_cncl)->get();
                if(count($gtc_offline_count) > 0){ 
                    if($gtc_offline->updated_at > $gtc_offline_count[0]->updated_at){  
                        DB::connection('mysql2')->table('gtc_cart_patient_confirm_cancel')->where('cart_cncl', $gtc_offline->cart_cncl)->update([    
                            'cart_cncl'=> $gtc_offline->cart_cncl, 
                        ...
                        DB::table('gtc_cart_patient_confirm_cancel')->where('cart_cncl', $gtc_offline_count[0]->cart_cncl)->update([  
                            'cart_cncl'=> $gtc_offline_count[0]->cart_cncl, 
                    ...
                    DB::connection('mysql2')->table('gtc_cart_patient_confirm_cancel')->insert([    
                        'cart_cncl'=> $gtc_offline->cart_cncl, 
                        'order_no'=>$gtc_offline->order_no,
                        'patient_id'=>$gtc_offline->patient_id,
                        'product'=>$gtc_offline->product,
        ...
        // syncronize gtc_cart_patient_confirm_cancel table from online to offline 
        $gtc_online = DB::connection('mysql2')->table('gtc_cart_patient_confirm_cancel')->get();  
        foreach($gtc_online as $gtc_online){  
            $gtc_online_count = DB::table('gtc_cart_patient_confirm_cancel')->where('cart_cncl', $gtc_online->cart_cncl)->get();
                if(count($gtc_online_count) > 0){
                    DB::table('gtc_cart_patient_confirm_cancel')->where('cart_cncl', $gtc_online->cart_cncl)->update([   
                        'cart_cncl'=> $gtc_online->cart_cncl, 
                    ...
                    DB::table('gtc_cart_patient_confirm_cancel')->insert([    
                        'cart_cncl'=> $gtc_online->cart_cncl, 
                        'order_no'=>$gtc_online->order_no,
                        'patient_id'=>$gtc_online->patient_id,
                        'product'=>$gtc_online->product,
