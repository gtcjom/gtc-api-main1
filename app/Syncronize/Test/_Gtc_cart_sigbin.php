...
use DB;
class _Gtc_cart_sigbin extends Model
{ 
    public static function gtc_cart_sigbin(){ 
        // syncronize gtc_cart_sigbin table from offline to online   
        $gtc_offline = DB::table('gtc_cart_sigbin')->get();  
        foreach($gtc_offline as $gtc_offline){  
            $gtc_offline_count = DB::connection('mysql2')->table('gtc_cart_sigbin')->where('rider', $gtc_offline->rider)->get();
                if(count($gtc_offline_count) > 0){ 
                    if($gtc_offline->updated_at > $gtc_offline_count[0]->updated_at){  
                        DB::connection('mysql2')->table('gtc_cart_sigbin')->where('rider', $gtc_offline->rider)->update([    
                            'rider'=> $gtc_offline->rider, 
                            ...
                            'unpaid_consultation'=>$gtc_offline->unpaid_consultation,
                            'status'=>$gtc_offline->status,
                            'order_toID'=>$gtc_offline->order_toID,
                            'vpharma_status'=>$gtc_offline->vpharma_status,
                            'rider_user_id'=>$gtc_offline->rider_user_id,
                            'rider_status'=>$gtc_offline->rider_status,
                            'start_delivery'=>$gtc_offline->start_delivery,
                            'complete_delivery'=>$gtc_offline->complete_delivery,
                            'created_at'=>$gtc_offline->created_at,
                            'updated_at'=>$gtc_offline->updated_at
                        ]);
                    } 
                    else{
                        DB::table('gtc_cart_sigbin')->where('rider', $gtc_offline_count[0]->rider)->update([  
                            'rider'=> $gtc_offline_count[0]->rider, 
                            ...
                            'unpaid_consultation'=>$gtc_offline_count[0]->unpaid_consultation,
                            ...
                            'rider_user_id'=>$gtc_offline_count[0]->rider_user_id,
                            'rider_status'=>$gtc_offline_count[0]->rider_status,
                            'start_delivery'=>$gtc_offline_count[0]->start_delivery,
                            'complete_delivery'=>$gtc_offline_count[0]->complete_delivery,
                            'created_at'=>$gtc_offline_count[0]->created_at,
                            'updated_at'=>$gtc_offline_count[0]->updated_at
                        ]);
                    }  
                }else{
                    DB::connection('mysql2')->table('gtc_cart_sigbin')->insert([    
                        'rider'=> $gtc_offline->rider, 
                        'order_no'=>$gtc_offline->order_no,
                        'patient_id'=>$gtc_offline->patient_id,
                        'product'=>$gtc_offline->product,
        ...
        // syncronize gtc_cart_sigbin table from online to offline 
        $gtc_online = DB::connection('mysql2')->table('gtc_cart_sigbin')->get();  
        foreach($gtc_online as $gtc_online){  
            $gtc_online_count = DB::table('gtc_cart_sigbin')->where('rider', $gtc_online->rider)->get();
                if(count($gtc_online_count) > 0){
                    DB::table('gtc_cart_sigbin')->where('rider', $gtc_online->rider)->update([   
                        'rider'=> $gtc_online->rider, 
                        ...
                        'unpaid_consultation'=>$gtc_online->unpaid_consultation,
                        'status'=>$gtc_online->status,
                        'order_toID'=>$gtc_online->order_toID,
                        'vpharma_status'=>$gtc_online->vpharma_status,
                        'rider_user_id'=>$gtc_online->rider_user_id,
                        'rider_status'=>$gtc_online->rider_status,
                        'start_delivery'=>$gtc_online->start_delivery,
                        'complete_delivery'=>$gtc_online->complete_delivery,
                        'created_at'=>$gtc_online->created_at,
                        'updated_at'=>$gtc_online->updated_at
                    ]); 
                }else{
                    DB::table('gtc_cart_sigbin')->insert([    
                        'rider'=> $gtc_online->rider, 
                        'order_no'=>$gtc_online->order_no,
                        'patient_id'=>$gtc_online->patient_id,
                        'product'=>$gtc_online->product,
